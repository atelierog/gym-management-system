...
export default function Login({onLogin}){
 ...
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(REMEMBER_KEY)||"null");if(saved?.id){setId(saved.id);if(saved?.password)setPassword(saved.password);setRemember(true)}}catch{}},[]);
 async function submit(e){
  ...
  if(remember){try{localStorage.setItem(REMEMBER_KEY,JSON.stringify({id:cleanId,password}))}catch{}}else{try{localStorage.removeItem(REMEMBER_KEY)}catch{}}
