...
export default function Login({onLogin}){
 ...
 const suspended=error==="This gym has been suspended.";
 ...
 function contactAtelierOG(){
  const role=copy?.label||"Gym User";
  const subject=encodeURIComponent(`Gym Manager access issue${routeGymSlug?` - ${routeGymSlug}`:""}`);
  const body=encodeURIComponent(`Hello Atelier OG,\n\nI am unable to access Gym Manager because my gym is suspended.\n\nRole: ${role}\nGym: ${routeGymSlug||"Not specified"}\nLogin ID: ${id.trim()||"Not specified"}\n\nPlease assist me with this issue.`);
 ...
 async function submit(e){
  ...
  try{
   ...
   if(platform?.status==="active"){
    ...
    if(routeGymSlug && profile.gym_slug){
      if(String(profile.gym_slug).toLowerCase()!==routeGymSlug){await signOut();throw new Error("This login link belongs to a different gym.")}
    }else if(routeGymSlug){
   ...
   const msg=raw.startsWith("This link is for the ")||raw==="This login link belongs to a different gym."||raw==="Your Gym Manager profile could not be found."||raw==="This gym has been suspended."?raw:raw==="This account is inactive. Contact your Gym Admin."?raw:"Invalid Login ID or password.";
   setError(msg);reportLoginError(msg,raw.includes("different gym")?"WRONG_GYM":raw.includes("This link")?"WRONG_ROLE":raw.includes("gym has been suspended")?"GYM_SUSPENDED":raw==="This account is inactive. Contact your Gym Admin."?"ACCOUNT_INACTIVE":"AUTHENTICATION");
  }
 }
 return <div className="gm-login-shell"><div className="gm-login-glow gm-login-glow-one"/><div className="gm-login-glow gm-login-glow-two"/><div className="gm-login-arc gm-login-arc-one"/><div className="gm-login-arc gm-login-arc-two"/><div className="gm-login-brand"><div className="gm-aog-mark">AOG</div><div><b>ATELIER OG</b><span>Business systems & automation</span></div></div><div className="gm-login-content"><div className="gm-login-logo-wrap"><img src="/gym-manager/icon.svg?v=7" alt="Gym Manager" className="gm-login-logo"/></div><div className="gm-login-title"><span>Gym</span> <strong>Manager</strong></div><div className="gm-login-tag">MANAGE · GROW · TOGETHER</div><form onSubmit={submit} className="gm-login-card"><div className="gm-login-heading"><div className="gm-eyebrow">{copy?.label.toUpperCase()||"YOUR GYM · YOUR CONTROL"}</div><h1>{copy?.title||"Welcome to Your Gym Manager"}</h1><p>{copy?.subtitle||"Sign in to manage your gym"}</p></div><label>Login ID<input value={id} onChange={e=>setId(e.target.value)} placeholder="Enter your Login ID" autoComplete="username"/></label><label>Password<div className="gm-password-wrap"><input type={showPassword?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Enter your password" autoComplete="current-password"/><button type="button" className="gm-password-toggle" onClick={()=>setShowPassword(v=>!v)} aria-label={showPassword?"Hide password":"Show password"}>{showPassword?<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3l18 18M10.6 10.6a3 3 0 004.2 4.2M9.9 5.2A10.8 10.8 0 0112 5c5 0 8.7 3.3 10 7a11.8 11.8 0 01-2.7 4.5M6.1 6.1C4.5 7.3 3.4 9 2 12c1.3 3.7 5 7 10 7 1.3 0 2.5-.2 3.6-.6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>:<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" strokeWidth="2"/></svg>}</button></div></label><label className="gm-remember"><input type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)}/><span>Remember me</span></label>{suspended?<div className="gm-suspended"><div className="gm-suspended-title">Gym Suspended</div><p>{routeRole==="owner"?"This gym has been suspended. Access to Gym Manager is unavailable for the Gym Owner.":routeRole==="trainer"?"This gym has been suspended. Access to Gym Manager is unavailable for the Trainer.":routeRole==="member"?"This gym has been suspended. Access to Gym Manager is unavailable for the Member.":"This gym has been suspended. Access to Gym Manager is unavailable."}</p><div className="gm-suspended-actions"><button type="button" className="gm-suspended-contact" onClick={contactAtelierOG}>Contact Atelier OG</button><button type="button" className="gm-suspended-ok" onClick={()=>setError("")}>OK</button></div></div>:error&&<div className="gm-error">{error}</div>}<button className="gm-signin" disabled={busy} aria-busy={busy}>{busy?<><span className="gm-signin-spinner" aria-hidden="true"/>Signing in…</>:"Sign in"}</button><div className="gm-login-footer"><span>Gym Manager</span><span>Powered by Atelier OG</span></div></form></div></div>
}
