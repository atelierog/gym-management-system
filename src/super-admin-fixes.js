...
function showFixDialog({title,gym,copy,danger=false,onConfirm,confirmText}){
 ...
 overlay.querySelector('.gymos-fix-confirm').onclick=async()=>{
  ...
  catch(err){errorEl.textContent=err?.message||'The action could not be completed.';errorEl.classList.add('is-visible');confirm.disabled=false;confirm.textContent=confirmText||'Confirm';busy=false}
 };
...
function handleAction(text){
 ...
 if(t==='suspend gym'){showFixDialog({title:'Suspend gym?',gym:currentGym,copy:'This will immediately block the Gym Owner, trainers, and members from accessing this gym. Existing gym data will be preserved and can be restored by the Super Admin.',danger:true,confirmText:'Suspend Gym',onConfirm:()=>updateGymStatus('suspended')});return true}
 if(t==='activate gym'){showFixDialog({title:'Restore gym access?',gym:currentGym,copy:'This will restore access to this gym for its Gym Owner, trainers, and members. Existing gym data will remain unchanged.',confirmText:'Restore Gym',onConfirm:()=>updateGymStatus('active')});return true}
 ...
 if(t==='activate owner access'||t==='activate gym owner access'||t==='activate gym owner access?'){showFixDialog({title:'Restore Gym Owner account?',gym:currentGym,copy:'This will restore sign-in access for the Gym Owner. The gym status will remain unchanged.',confirmText:'Restore Owner',onConfirm:()=>updateOwnerStatus('active')});return true}
 if(t==='delete gym & all data'){showFixDialog({title:'Delete gym permanently?',gym:currentGym,copy:'This permanently removes the gym, its owner account and all related gym data. This action cannot be undone.',danger:true,confirmText:'Delete Gym & All Data',onConfirm:deleteGym});return true}
 return false;
}
