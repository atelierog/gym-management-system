...
do $$ begin
  if not exists (select 1 from pg_constraint where conname='gyms_radius_chk') then
    alter table public.gyms add constraint gyms_radius_chk check (allowed_radius_m between 10 and 5000);
  end if;
  if not exists (select 1 from pg_constraint where conname='gyms_auto_checkout_minutes_chk') then
    alter table public.gyms add constraint gyms_auto_checkout_minutes_chk check (auto_checkout_minutes between 15 and 1440);
  end if;
  if not exists (select 1 from pg_constraint where conname='membership_plans_duration_chk') then
    alter table public.membership_plans add constraint membership_plans_duration_chk check (duration_months in (1,3,6,12));
  end if;
  if not exists (select 1 from pg_constraint where conname='memberships_dates_chk') then
    alter table public.memberships add constraint memberships_dates_chk check (expiry_date >= start_date);
  end if;
  if not exists (select 1 from pg_constraint where conname='attendance_checkout_time_chk') then
    alter table public.attendance add constraint attendance_checkout_time_chk check (check_out is null or check_out >= check_in);
  end if;
end $$;
...
with check (
  memberships.gym_id=(select private.current_gym_id())
  ...
  and (memberships.plan_id is null or exists(select 1 from public.membership_plans mp where mp.id=memberships.plan_id and mp.gym_id=memberships.gym_id))
);
...
using(memberships.gym_id=(select private.current_gym_id()) and (select private.current_role())='admin')
with check (
  memberships.gym_id=(select private.current_gym_id())
  and (select private.current_role())='admin'
  and exists(select 1 from public.profiles p where p.id=memberships.member_id and p.gym_id=memberships.gym_id and p.role='member')
...
with check (
  payments.gym_id=(select private.current_gym_id())
  and (select private.current_role())='admin'
  and exists(select 1 from public.profiles p where p.id=payments.member_id and p.gym_id=payments.gym_id and p.role='member')
  and (payments.membership_id is null or exists(select 1 from public.memberships m where m.id=payments.membership_id and m.gym_id=payments.gym_id and m.member_id=payments.member_id))
);

drop policy if exists payments_admin_update on public.payments;
create policy payments_admin_update on public.payments for update to authenticated
using(payments.gym_id=(select private.current_gym_id()) and (select private.current_role())='admin')
with check (
  payments.gym_id=(select private.current_gym_id())
  and (select private.current_role())='admin'
  and exists(select 1 from public.profiles p where p.id=payments.member_id and p.gym_id=payments.gym_id and p.role='member')
...
begin
  ...
  where m.status='active' and m.expiry_date=current_date+1
