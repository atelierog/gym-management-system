...
async function sendOwnerWelcomeEmail(args:{gymId:string,gymSlug:string,gymName:string,ownerName:string,ownerEmail:string,ownerPhone:string,loginId:string,password:string}) {
  ...
  try{
    ...
    const text=[
      `Welcome to Gym Manager, ${args.ownerName}.`,
      "",
    ...
    const html=`<div style="margin:0;background:#f3f4f6;padding:32px 12px;font-family:Arial,Helvetica,sans-serif;color:#111827">
      <div style="max-width:640px;margin:auto;background:#fff;border:1px solid #e5e7eb;border-radius:18px;overflow:hidden">
        <div style="background:#0e1626;color:#fff;padding:30px 34px">
          <div style="font-size:26px;font-weight:800;letter-spacing:1px">GYM MANAGER</div>
        ...
        <div style="padding:32px 34px">
          ...
          <p style="color:#6b7280;line-height:1.6;margin:0 0 24px">Your Gym Manager owner account has been created successfully by Atelier OG.</p>
          ...
          <div style="background:#0e1626;color:#fff;border-radius:14px;padding:22px;margin-bottom:20px">
            <div style="font-size:11px;letter-spacing:1.5px;color:#cbd5e1;font-weight:700;margin-bottom:14px">GYM OWNER LOGIN</div>
          ...
          <div style="text-align:center">
            <a href="${esc(roleUrl)}" style="display:inline-block;background:#0e1626;color:#fff;text-decoration:none;padding:13px 24px;border-radius:10px;font-weight:700">Open Gym Manager</a>
            <div style="margin-top:10px;font-size:11px;color:#6b7280;word-break:break-all">${esc(roleUrl)}</div>
          </div>
          <div style="margin-top:24px;padding:16px;border-radius:12px;background:#fffaf0;border:1px solid #ead7a3;color:#6b5a2a;font-size:12px;line-height:1.6">
            This link is for the <strong>Gym Owner</strong> role for ${esc(args.gymName)}. Please change the temporary password after your first login.
          </div>
        </div>
        <div style="padding:18px 34px;background:#f8fafc;color:#9ca3af;font-size:11px">Atelier OG Gym Manager • Business management system</div>
      </div>
    </div>`;
...
Deno.serve(async (req)=>{
  ...
  let gymSlug=base;
  for(let i=2;i<=999;i++){
    const {data:used}=await admin.from("gyms").select("id").eq("slug",gymSlug).limit(1);
    if(!used?.length) break;
    gymSlug=`${base}-${i}`;
  }

  const loginBase=(requestedLogin||name).toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,8)||"GYM";
  let loginId=requestedLogin.toUpperCase().replace(/\s+/g,"").replace(/[^A-Z0-9_-]/g,"");
  if(!loginId) { for(let i=1;i<=999;i++){ const candidate=loginBase+String(i).padStart(2,"0"); const {data:used}=await admin.from("profiles").select("id").eq("login_id",candidate).limit(1); if(!used?.length){loginId=candidate;break;} } }
  ...
  const {data:gym,error:ge}=await admin.from("gyms").insert({name,slug:gymSlug}).select().single();
  ...
  return Response.json({gym,owner:profile,temporary_password:password,password_change_required:true,role_login_url:`${APP_BASE_URL}/${gym.slug}/owner`,email_sent:emailResult.sent,email_status:emailStatus},{headers:{...CORS,"Content-Type":"application/json"}});
});
