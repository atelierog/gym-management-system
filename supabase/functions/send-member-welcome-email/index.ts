...
Deno.serve(async(req)=>{
  ...
  const durationValue=String(b.duration_value||b.duration_months||""); const durationUnit=String(b.duration_unit||"month"); const durationLabel=durationValue+" "+durationUnit+(Number(durationValue)===1?"":"s");
  const html=`<div style="font-family:Arial,sans-serif;max-width:680px;margin:auto;color:#111827"><h1>${esc(b.gym_name||"Gym")}</h1><p>Welcome to the gym. Your membership has been registered successfully.</p><h2>Membership</h2><p><b>Member:</b> ${esc(b.member_name)}<br><b>Member ID:</b> ${esc(b.member_id)}<br><b>Duration:</b> ${esc(durationLabel)}<br><b>Start:</b> ${esc(b.start_date)}<br><b>Expiry:</b> ${esc(b.expiry_date)}<br><b>Amount:</b> INR ${esc(b.amount)}<br><b>Payment:</b> ${esc(b.payment_method)}<br><b>Receipt:</b> ${esc(b.receipt_no)}</p><h2>Login details</h2><p><b>Login ID:</b> ${esc(b.member_id)}<br><b>Temporary password:</b> ${esc(b.password)}</p><p>Please change your password after your first login and do not share your login details.</p><p>Your membership document is attached as a PDF.</p><p>- GYMOS</p></div>`;
